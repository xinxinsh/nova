...
class ServersController(wsgi.Controller):
    ...
    def update(self, req, id, body):
        ...
        try:
            ...
            instance.save()
            self.compute_api.operation_log_about_instance(ctxt, 'Succeeded')
            return self._view_builder.show(req, instance,
                                           extend_address=False)
        except exception.InstanceNotFound:
            self.compute_api.operation_log_about_instance(ctxt, 'Failed')
    ...
    def delete(self, req, id):
        ...
        try:
            ...
            if not action_result:
                self.compute_api.operation_log_about_instance(
                    req.environ['nova.context'],
                    'Failed')

    def _delete_snap(self, req, id):
        # delete snap before delete instance
        lsi_api = live_snapshot_instance.\
            LiveSnapshotInstanceController(self.extension_info)
        req.GET["instance_id"] = id
        snaps = lsi_api.index(req)['snapshot_list']
        if len(snaps) > 0:
            for snap in snaps:
                LOG.info("Deleting snap %s", snap['snapshot_id'])
                try:
                    lsi_api.delete(req, snap['snapshot_id'])
                except Exception:
                    msg = (_("Deleting snap %s error"),
                            snap['snapshot_id'])
                    raise exc.HTTPBadRequest(explanation=msg)
    ...
    def _get_server_search_options(self, req):
        ...
        opt_list = ('reservation_id', 'name', 'status', 'image', 'flavor',
                    'ip', 'changes-since', 'all_tenants', 'iso')
