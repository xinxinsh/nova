...
def load_from_conf_data(conf_data):
    ...
    for (key, value) in attribs.items():
        try:
            ...
            LOG.info(_LI('Encountered unknown LPAR attribute: %s\n'
...
class LPAR(object):
    ...
    def __init__(self, **kwargs):
        attrs = {}
        p_attrs = {}
        for attr in self.LPAR_ATTRS:
            attrs.setdefault(attr, None)
        for p_attr in self.LPAR_PROFILE_ATTRS:
            p_attrs.setdefault(p_attr, None)
        self.attributes = attrs
        self.profile_attributes = p_attrs
    ...
    def to_string(self, exclude_attribs=None):
        conf_data = []
        if exclude_attribs is None:
            exclude_attribs = []
