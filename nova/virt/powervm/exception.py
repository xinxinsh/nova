...
class PowerVMInvalidLUNPathInfoMultiple(exception.NovaException):
    msg_fmt = _("More than one LUN in the path info. "
                "conn_info %(conn_info)s, lspath "
                "output: %(outputs)s")


class PowerVMInvalidLUNPathInfoNone(exception.NovaException):
    msg_fmt = _("No disk has been found based on the "
                "conn_info %(conn_info)s, lspath "
                "output: %(outputs)s")
