...
class PowerVMLocalVolumeAdapter(PowerVMDiskAdapter):
    ...
    def create_volume_from_image(self, context, instance, image_id):
        ...
        # size_gb = max(instance_type['root_gb'],
        #                constants.POWERVM_MIN_ROOT_GB)
        size_gb = max(instance.root_gb, constants.POWERVM_MIN_ROOT_GB)
    ...
    def create_image_from_volume(self, device_name, context,
        ...
        dest_file_path = common.aix_path_join(
            CONF.powervm.powervm_img_remote_path, image_id)
        ...
        try:
            ...
            LOG.warn(_LW("Failed to clean up snapshot file %s"),
                     snapshot_file_path)
    ...
    def detach_volume_from_vhost(self, volume_info):
        lv_name = volume_info["device_name"]
        cmd = self.command.rmvdev('-vdev %s' % lv_name)
        self.run_vios_command(cmd)
    ...
    def discovery_device(self, volume_data):
        target_portal = volume_data['target_portal']
        target_iqn = volume_data['target_iqn']
        ip = target_portal.split(':')[0]
        port = target_portal.split(':')[1]
        command = ('echo "%s %s %s" >> /etc/iscsi/targets' %
                   (ip, port, target_iqn))
        self.run_vios_command_as_root(command)
        self.run_vios_command_as_root('cfgmgr')

    def cancel_discovery_device(self, volume_name):
        command = ('sed \'/%s/d\''
                   ' /etc/iscsi/targets >/etc/iscsi/targets.new' % volume_name)
        self.run_vios_command_as_root(command)
        cmd = 'cat /etc/iscsi/targets.new >/etc/iscsi/targets'
        self.run_vios_command_as_root(cmd)

    def get_initiator_name(self):
        cmd = "lsdev -C -t iscsi -S available"
        output = self.run_vios_command_as_root(cmd)
        if output:
            iscsi_name = output[0].split(" ")[0]
        else:
            iscsi_name = None
        if iscsi_name:
            cmd = ("lsattr -El %s -a initiator_name" % iscsi_name)
            initiator_info = self.run_vios_command_as_root(cmd)
            if initiator_info:
                return initiator_info[0].split(" ")[1]
        return None
