...
class API(base.Base):
    ...
    def snapshot_for_instance(self, context, instance, name,
        ...
        :param name: name of the backup or snapshot
        :param extra_properties: dict of extra image properties to include
        :param memory_snapshot: support the memory snapshot of instance
        ...
        if memory_snapshot and properties['system_snapshot'] == "True":
            if instance.vm_state == 'active':
                ...
                self.compute_rpcapi.instance_actions(context,
                                                     instance,
                                                     action='paused')
            elif instance.vm_state == 'paused':
                vm_active = False
            else:
                msg = _('Instance is not running or paused')
    ...
    @check_instance_cell
    @check_instance_state(vm_state=[vm_states.ACTIVE, vm_states.STOPPED])
    def resize_local(self, context, instance, flavor_id=None,
                 clean_shutdown=True, **extra_instance_updates):
        """Local Resize  a running instance.

        If flavor_id is not None, the instance should
        be resized to the new flavor_id on the same host.
        """
        self._check_auto_disk_config(instance, **extra_instance_updates)

        current_instance_type = instance.get_flavor()

        # If flavor_id is not provided, only hard reboot instance
        if not flavor_id:
            LOG.debug("flavor_id is None. Assuming reboot --hard.",
                      instance=instance)
        ...
        self.compute_rpcapi.resize_local_instance(context, instance,
                extra_instance_updates, flavor=new_instance_type,
        ...
        if(image_meta['properties']['image_type'] == 'snapshot_for_instance'
           ...
           and 'instance_uuid' in image_meta['properties']):
            ...
            if image_meta['properties']['is_volume'] == 'True':
                snapshot_info['disk_type'] = 'volume'
        ...
        if(image_meta['status'] == 'active'
           and image_meta['properties']['image_type'] ==
            ...
            if delete_error is not True:
                try:
                    image_main = image_service.show(context, image_id)
                    if image_main['status'] == 'active':
                        image_service.delete(context, image_id)

                    if image_main['status'] == 'error_deleting':
                        msg_snapshot = _("main snapshot %s is in "
                                         "error_deleting status! ") % image_id
                        msg = msg + msg_snapshot
                except Exception:
                    msg_snapshot = _("delete main snapshot %s error! ") % \
                                   image_id
                    return msg_snapshot
